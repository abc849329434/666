var rule = {
    ...
    一级: '',
    二级: $js.toString(() => {
        let parts = input.replace(rule.host + "/", "")
            .split('~~');
        ...
        let data = {
            ...
            vod_pic: rule.pic,
            vod_play_from: '推送"jar": "http://123.207.224.130:7033/ZC4K.jar",
    ...
    搜索:  $js.toString(() => {
        ...
        //  console.log(html);
        let strArray = html1.split('\n ');
        for (let s of strArray) {
            if (s.includes('
            data ') && !s.includes('
            DONE ')) {
                let jsonStr = s.replace(/^data:/, '
            ');
                try {
