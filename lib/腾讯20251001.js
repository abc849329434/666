...
var rule = {
    ...
    搜索: $js.toString(() => {
    let d = [];
    let searchKeyword = input.split("/")[3] || input;
    
    let html = vod1(searchKeyword);
    let json = JSON.parse(html);
    ...
    if (json.data && json.data.normalList && json.data.normalList.itemList) {
        json.data.normalList.itemList.forEach(function(it) {
            try {
                if (it.doc && it.doc.id && it.doc.id.length > 11) {
                    d.push({
                        title: it.videoInfo.title || "未知标题",
                        img: it.videoInfo.imgUrl || "",
                        url: "https://node.video.qq.com/x/api/float_vinfo2?cid=" + it.doc.id,
                        content: it.videoInfo.description || "",
                        desc: it.videoInfo.subTitle || ""
                    });
                }
            } catch (e) {
                console.log("处理normalList项出错:", e);
            }
        });
    }
    
    if (json.data && json.data.areaBoxList && json.data.areaBoxList.length > 0) {
        json.data.areaBoxList.forEach(function(areaBox) {
            if (areaBox.itemList) {
                areaBox.itemList.forEach(function(it) {
                    try {
                        if (it.doc && it.doc.id && it.doc.id.length > 11 && 
                            it.videoInfo.title && it.videoInfo.title.includes(searchKeyword)) {
                            d.push({
                        ...
                        console.log("处理areaBoxList项出错:", e);
                    }
                });
            }
        });
    ...
    if (json.data && json.data.ipBoxList && json.data.ipBoxList.length > 0) {
        json.data.ipBoxList.forEach(function(ipBox) {
            if (ipBox.itemList) {
                ipBox.itemList.forEach(function(it) {
                    try {
                        if (it.doc && it.doc.id) {
                            d.push({
                                title: it.videoInfo.title || it.videoInfo.seriesName || "未知标题",
                                img: it.videoInfo.imgUrl || it.videoInfo.coverUrl || "",
                                url: "https://node.video.qq.com/x/api/float_vinfo2?cid=" + it.doc.id,
                                content: it.videoInfo.description || "",
                                desc: "IP内容"
                            });
