...
class Spider(Spider):
    ...
    def init(self, extend):
        try:
            ...
            self.baseUrl = 'https://emby-sg.nubebelle.vip:443'
            self.username = 'tgccc677dd'
            self.password = 'd4405c7e'
            self.proxy = '0'
            self.thread = 0
    ...
    def _record_playback_start(self, embyInfos, item_id, media_source):
        ...
        # 发送播放开始请求
        play_url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/Sessions/Playing"
        try:
    ...
    def _record_playback_progress(self, embyInfos, item_id, media_source, session_id, position_seconds):
        ...
        # 发送播放进度请求
        progress_url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/Sessions/Playing/Progress"
        try:
            response = requests.post(
                progress_url, 
                json=progress_data, 
    ...
    def _record_playback_stop(self, session_id):
        ...
        # 发送播放停止请求
        stop_url = f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/Sessions/Playing/Stopped"
        try:
            response = requests.post(
                stop_url, 
                json=stop_data, 
    ...
    def getAccessToken(self):
        ...
        r = requests.post(
            f"http://127.0.0.1:9966/api/proxy/{self.baseUrl}/emby/Users/AuthenticateByName", 
            json=auth_data, 
            headers=header, 
            timeout=120, 
            proxies={"http": self.proxy, "https": self.proxy}
