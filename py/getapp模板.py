# coding = utf-8
# !/usr/bin/python
# 新时代青年 2025.06.22 getApp第二版
# 本作品收集与互联网,纯属个人爱好,请勿用于商业用途.仅供测试与学习交流,下载后请于24小时删除
        ...
        for line in kjson1['vod_play_list']:
            ...
            for vod in line['urls']:
                ...
                if '.m3u8' in  kurl:
                    ...
                    if 'm3u8' not in kurl :
                        # if 'm3u8' not in vod['url']:
                        #     kurls += str(vod['name']) + '$' + vod['url'] + '#'
                        token = 'token+' + vod['token']
                        kurls += str(vod['name']) + '$' + parse + ',' + vod['url'] + ',' + token + ',' + player_parse_type + '#'
            kurls = kurls.rstrip('#')
            play_url += kurls + '$$$'
