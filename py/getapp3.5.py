...
class Spider(Spider):
    ...
    def init(self, extend=''):
        ...
        host = ext['host']
        self.xurl = self._resolve_host(host)
    ...
    def homeContent(self, filter):
        ...
        for t in data.get('type_list', []):
            ...
            if tname in {'全部','QQ','juo.one'} or '企鹅群' in tname:
                continue
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        res = self.post(
            self._build_url('typeFilterVodList'),
            headers=self._build_headers(),
            data=payload,
            verify=False
        )
        ...
        if vod_play_list and not vod_play_url_list:
            ...
            for line in vod_play_list:
                ...
                for ep in line.get('urls', []):
                    ...
                    token = ep.get('token', '')
                    kurls.append(
                        f"{ep_name}${parse},{ep['url']},token+{token},{player_parse_type},{parse_type}"
                    )
                play_url += '#'.join(kurls) + '$$$'
                line_id += 1
        else:
            code_to_id = {
                ps['player_code']: ps['id']
                for ps in player_source_list
                if 'player_code' in ps and 'id' in ps
            }
            player_map = {
                p['player_code']: p
                for p in player_source_list
                if 'player_code' in p
            }
            if self.sort_rule:
                def sorter(item):
            ...
            for play in vod_play_url_list:
                ...
                urls = play.get('urls', [])
                kurls = [
                    f"{ep.get('name', f'第{ei+1}集')}${did},{pcode},{source_id},{ei},{player_parse_type},{parse_type}"
                    for ei, ep in enumerate(urls)
                ]
                play_url += '#'.join(kurls) + '$$$'
                line_id += 1
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        payload = {
            'vod_id': vod_id,
            'player_source_id': payload_sid,
            'episode_index': ei
        }
        try:
            resp = self.post(
                self._build_url('vodParse'),
                headers=self._build_headers(),
                data=payload,
                verify=False
            )
        ...
        if isinstance(raw, dict):
            for k in ('url', 'play_url', 'video_url', 'src'):
                if k in raw and raw[k]:
                    return self._extract_url(raw[k])
            if 'data' in raw:
                return self._extract_url(raw['data'])
            if 'json' in raw:
                return self._extract_url(raw['json'])
        return ''
