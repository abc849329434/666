...
iwj8gxKr0ls+P4TUT9S7tpqSu3CVJqZKuaWFnthpZdPELD/QHfXaRrDEAeYTT5dG
tPn93UE8wsUOS5ceYhJsRQwz2beqv+XJB/LAcPt+r+MJu1A280k7T38htXuj+CW9
ymQIDAQAB
-----END PUBLIC KEY-----'''
...
class Spider(Spider):
    ...
    # ========== 新增：本地解密 vod.baidu.com 异或链路 ==========
    def vod_baidu_xor(self, cipher_url: str) -> str:
        try:
            raw = cipher_url.replace('https://vod.baidu.com/', '').strip()
            if not raw:
                return None
            key = str(int(time.time()))[:8]                      # 8 位时间戳
            mid = base64.b64decode(raw)
            out = bytes(b ^ ord(key[i % 8]) for i, b in enumerate(mid))
            real_url = base64.b64decode(out).decode('utf-8')
