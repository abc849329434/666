...
class Spider(Spider):
    ...
    def init(self, extend=''):
        ...
        self.raw_play_url = int(
            self.FIXED_CONFIG.get('RawPlayUrl', 0)
        )
    ...
    def homeContent(self, filter):
        ...
        for item in data['list']:
            ...
            has_filter = any(
                k in item["type_extend"]
                and item["type_extend"][k].strip()
                for k in keys
            )
        ...
        for i in data['vod_url_with_player']:
            ...
            for u in i['url'].split('#'):
                ...
                if u:
                    ...
                    real_url = self.lvdou(url)

                    urls.append(
                        f"{name}${real_url}"
                    )

            show_name = self.PLAY_NAME_POOL[
                name_idx % len(self.PLAY_NAME_POOL)
            ]
    ...
    def check_paly_url(self, text):
        ...
        return bool(re.search(
            r'https?://[^\s]+?\.(mp4|m3u8|flv|avi|mkv|ts|mov|wmv|webm)(\?.*)?$',
            text,
            re.I
        ))
