...
class DecryptShijie:
    def __call__(self, str_data, z=False, filter=False):
        return self.decrypt(str_data, z, filter)

    @staticmethod
    def filter_string(original_str: str) -> str:
        
        remove_chars = set(r"%!_*&>,).|?$(){[];^~()<`#@}- ")
        return ''.join([char for char in original_str if char not in remove_chars])

    @staticmethod
    def decrypt(str_data, z=False, filter=False):
...
class Spider(Spider):
    ...
    def ry_jx(self, params):
        ...
        filter_flag = params.get('filter')

        if filter_flag is not None:
            if isinstance(filter_flag, str):
                filter_flag = filter_flag.strip().lower() in ('1', 'true', 'yes', 'on')
            elif isinstance(filter_flag, int):
                filter_flag = filter_flag == 1
            else:
                filter_flag = bool(filter_flag)
        else:
            filter_flag = False
        ...
        for i in range(retry+1):
            ...
            video_url = self.common_decrypt(dat_vurl, filter_flag)
    ...
    def common_decrypt(self, dat_vurl, filter=False):
        
        if filter:

            if 'https://baidu.con' in dat_vurl:

                parts = dat_vurl.split('https://baidu.con', 1)
                if len(parts) > 1:

                    after_baidu = parts[1]

                    end_marker = after_baidu.find('="')
                    if end_marker != -1:

                        part_to_filter = after_baidu[:end_marker]
                        filtered_part = DecryptShijie.filter_string(part_to_filter)
       
                        dat_vurl = parts[0] + 'https://baidu.con' + filtered_part + after_baidu[end_marker:]
                    else:

                        filtered_part = DecryptShijie.filter_string(after_baidu)
                        dat_vurl = parts[0] + 'https://baidu.con' + filtered_part
        ...
        if re.match(r'^https?://baidu\.con/', dat_vurl, re.I):
            ...
            if result: return result
        elif 'lvDou+' in dat_vurl or 'lvdou+' in dat_vurl:
            
            result = DecryptShijie.decrypt(dat_vurl, filter=False)
            if result: return result
