...
class Spider(Spider):
    ...
    def init(self, extend=''):
        try:
            self.proxy_url = f'{self.getProxyUrl(True)}'
        except Exception:
            self.proxy_url = f'http://127.0.0.1:9978/proxy?do=py'

    def localProxy(self, params):
        try:
            if params.get('v'):
                data = self.jx(params)
            elif params.get('m3u8'):
                data = self.m3u8(params)
            else:
                data = '请按要求传参'
    ...
    def m3u8(self, params):
        m3u8 = params.get('m3u8')
        if not m3u8: return self.json_encode({"code": "400", "success": "0", "msg": "URL或API为空"})
        url = unquote_plus(m3u8)
        return self.fetch(url, headers=self.xm_headers, verify=False).text

    def jx(self, params):
        v = params.get('v')
        if not v: return self.json_encode({"code": "400", "success": "0", "msg": "URL或API为空"})
        ...
        qy_headers = {**self.headers, 'origin': 'https://www.iqiyi.com', 'referer': 'https://www.iqiyi.com/', 'sec-fetch-site': 'same-site'}
        response = self.fetch(f'https://data.video.iqiyi.com/v.f4v?random={int(time.time() * 1000)}', headers=qy_headers, verify=False).json()
        ...
        xm_headers2 = {**self.xm_headers, 'Accept': 'application/json, text/javascript, */*; q=0.01'}
        data = ''
        api_num = 1
        for api in ['https://202.189.8.170/Api.js', 'https://cache.hls.one/Api.js']:
            try:
                response = self.post(api, data=payload, headers=xm_headers2, verify=False, timeout=10).json()
        ...
        play_url =  data3.get('url','')
        if not play_url.startswith('http') or play_url == 'https://kjjsaas-sh.oss-cn-shanghai.aliyuncs.com/u/3401405881/20240818-936952-fc31b16575e80a7562cdb1f81a39c6b0.mp4':
            return self.json_encode({"code": "-3","success": "0","msg": "解析失败", "from":v})
        if '?vkey=' in  play_url.lower() and '/cache/' in play_url.lower():
            if '.mp4' in play_url:
                headers = {
                    **self.headers,
                    'priority': 'i',
                ...
                play_url = self.raw_url(play_url, headers)
            elif '.m3u8' in play_url:
                if re.match(r'^https://(?:m|www)\.mgtv\.com/', v, re.I):
                    play_url = self.raw_url(play_url, self.xm_headers)
                else:
                    play_url = f'{self.proxy_url}&siteKey=proxy_xmflvjx&m3u8={quote_plus(play_url)}'
        data3['url'] = play_url
        if re.match(r'^https://(?:m|www)\.bilibili\.com/', v, re.I):
            ...
            data3['User-Agent'] = self.headers['User-Agent']
        data3['from'] = v
        data3['api'] = api_num
        return self.json_encode(data3)

    def raw_url(self,original_url,headers):
        try:
            response = self.fetch(original_url,headers=headers,allow_redirects=False,stream=True,verify=False,timeout=8)
