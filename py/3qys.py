...
class Spider(Spider):
    ...
    def playerContent(self, flag, vid, vip_flags):
        ...
        if need_parse == '1':
            try:
                response = self.fetch(f'{self.host}/api.php/decode/url/?url={raw_url}&vodFrom={play_from}',headers=self.headers,timeout=30,verify=False).json()
        ...
        if not url:
            ...
            if re.search(r'(?:www\.iqiyi|v\.qq|v\.youku|www\.mgtv|www\.bilibili)\.com', raw_url):
                jx = 1
        return { 'jx': jx, 'parse': 0, 'url': url, 'header': {'User-Agent': 'com.sunshine.tv/1.2.0 (Linux;Android 15) AndroidXMedia3/1.4.1','Accept-Encoding': 'gzip'}}
